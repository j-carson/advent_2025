...
def solve(input_data, n_to_connect):
    distances = {}
    ...
    for _, keys in lengths:
        ...
        n_connected += 1
        if n_connected == n_to_connect:
            break

    ic(groups)
    unique_groups = []
    for key in range(n_points):
        if key in groups:
            unique_groups.append(groups[key])
            for ikey in groups[key]:
                groups.pop(ikey)

    ic(unique_groups)

    group_sizes = sorted(((len(g), g) for g in unique_groups), reverse=True)
    ic(group_sizes)
    return group_sizes[0][0] * group_sizes[1][0] * group_sizes[2][0]
...
def test_samples(sample_data, sample_solution) -> None:
    assert solve(sample_data, 10) == sample_solution
